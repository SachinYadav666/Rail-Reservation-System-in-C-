...
NODE reserve(NODE head[], int number[], int count, int num)
{
    ...
    for (z = 0; z < n1; z++)
    {
        ...
        do
        {
            cout << "\nReservation for the following dates are available : ";
            cout << "\nFrom 12/05/2024 till 30/07/2024\n";
            cout << "\nEnter the date for which the Reservation must be done (DD MM): ";
            cin >> date >> month;
   ...
   if(n == 1)
{
    ...
    string id; // Declare id variable here

    // Moved the instructions and input prompt outside the commented-out block
    cout << "Instructions:\n";
    cout << "Click on the new window which is opened.\nAfter paying the money please enter the transaction id or UTR number.\nNOTE: Do not close the application till you enter the id or number!\n";
    ...
    if(!outFILE)
    {
        cout << "Error opening paymentDetails.txt file!\n";
        return 1;
    }
...
{  
    ...
    while(true)
    {
        ...
        cout<<"[ NOTE : This option is for regular travelers who are familiar with the routes to speed up their process! ]\n";
